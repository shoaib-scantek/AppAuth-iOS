...
@implementation SCTKTokenRequest
...
- (instancetype)initWithConfiguration:(nonnull SCTKServiceConfiguration *)configuration
...
- (instancetype)initWithConfiguration:(SCTKServiceConfiguration *)configuration
  ...
  if (self) {
    ...
    if ([_grantType isEqual:SCTKGrantTypeAuthorizationCode]) {
      // redirect URI must not be nil
      if (!_redirectURL) {
        [NSException raise:SCTKOAuthExceptionInvalidTokenRequestNullRedirectURL
                    format:@"%@", SCTKOAuthExceptionInvalidTokenRequestNullRedirectURL, nil];
...
- (NSURLRequest *)URLRequest {
  ...
  SCTKURLQueryComponent *bodyParameters = [self tokenRequestBody];
  ...
  if (_clientSecret) {
    ...
    NSString *encodedClientID = [SCTKTokenUtilities formUrlEncode:_clientID];
    NSString *encodedClientSecret = [SCTKTokenUtilities formUrlEncode:_clientSecret];
