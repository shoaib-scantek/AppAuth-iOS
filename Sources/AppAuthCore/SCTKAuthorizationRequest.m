/*! @file SCTKAuthorizationRequest.m
 ...
 */
static NSString *const SCTKOAuthUnsupportedResponseTypeMessage =
 ...
 */
NSString *const SCTKOAuthorizationRequestCodeChallengeMethodS256 = @"S256";

@implementation SCTKAuthorizationRequest
...
{
  NSString *codeIdToken = [@[SCTKResponseTypeCode, SCTKResponseTypeIDToken]
                           componentsJoinedByString:@" "];
  NSString *idTokenCode = [@[SCTKResponseTypeIDToken, SCTKResponseTypeCode]
                           componentsJoinedByString:@" "];

  return [responseType isEqualToString:SCTKResponseTypeCode]
    ...
    additionalParameters:(nullable NSDictionary<NSString *, NSString *> *)additionalParameters
{
  ...
  if (self) {
    ...
    if (![[self class] isSupportedResponseType:_responseType]) {
      NSAssert(NO, SCTKOAuthUnsupportedResponseTypeMessage, _responseType);
      return nil;
    }
    _state = [state copy];
    _nonce = [nonce copy];
...
- (instancetype)
   initWithConfiguration:(SCTKServiceConfiguration *)configuration
                clientId:(NSString *)clientID
  ...
  return [self initWithConfiguration:configuration
                        ...
                        clientSecret:clientSecret
                               scope:[SCTKScopeUtilities scopesWithArray:scopes]
                 ...
                 codeChallengeMethod:SCTKOAuthorizationRequestCodeChallengeMethodS256
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
  SCTKServiceConfiguration *configuration =
      [aDecoder decodeObjectOfClass:[SCTKServiceConfiguration class]
...
+ (nullable NSString *)generateCodeVerifier {
  return [SCTKTokenUtilities randomURLSafeStringWithSize:kCodeVerifierBytes];
}

+ (nullable NSString *)generateState {
  return [SCTKTokenUtilities randomURLSafeStringWithSize:kStateSizeBytes];
}
...
+ (nullable NSString *)codeChallengeS256ForVerifier:(NSString *)codeVerifier {
  ...
  NSData *sha256Verifier = [SCTKTokenUtilities sha256:codeVerifier];
  return [SCTKTokenUtilities encodeBase64urlNoPadding:sha256Verifier];
}
...
#pragma mark - SCTKExternalUserAgentRequest
