/*! @file SCTKExternalUserAgentSession.h
 ...
 */
@protocol SCTKExternalUserAgentSession <NSObject>
...
/*! @brief Cancels the code flow session, invoking the request's callback with a cancelled error.
    @remarks Has no effect if called more than once, or after a
        @c SCTKExternalUserAgentSession.resumeExternalUserAgentFlowWithURL: message was received.
        ...
        @c SCTKAuthorizationService.presentAuthorizationRequest:presentingViewController:callback:
 */
- (void)cancel;
...
/*! @brief @c SCTKExternalUserAgent or clients should call this method when the
