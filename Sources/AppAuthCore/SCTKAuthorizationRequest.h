/*! @file SCTKAuthorizationRequest.h
...
// These files only declare string constants useful for constructing a @c SCTKAuthorizationRequest,
// so they are imported here for convenience.
#import "SCTKExternalUserAgentRequest.h"
#import "SCTKResponseTypes.h"
#import "SCTKScopes.h"
 ...
 */
extern NSString *const SCTKOAuthorizationRequestCodeChallengeMethodS256;
 ...
 */
@interface SCTKAuthorizationRequest :
    NSObject<NSCopying, NSSecureCoding, SCTKExternalUserAgentRequest>
...
/*! @brief Designated initializer.
    ...
    @param nonce String value used to associate a Client session with an ID Token. Can be set to nil
        if not using OpenID Connect, although pure OAuth servers should ignore params they don't
        understand anyway.
    @param codeVerifier The PKCE code verifier. See @c SCTKAuthorizationRequest.generateCodeVerifier.
    @param codeChallenge The PKCE code challenge, calculated from the code verifier such as with
        @c SCTKAuthorizationRequest.codeChallengeS256ForVerifier:.
    @param codeChallengeMethod The PKCE code challenge method.
        ::OIDOAuthorizationRequestCodeChallengeMethodS256 when
        @c SCTKAuthorizationRequest.codeChallengeS256ForVerifier: is used to create the code
        challenge.
    @param additionalParameters The client's additional authorization parameters.
 */
- (instancetype)
...
/*! @brief Creates a PKCE S256 codeChallenge from the codeVerifier.
    ...
    @details Generate a secure code verifier to pass into this method with
        @c SCTKAuthorizationRequest.generateCodeVerifier. The matching @c #codeChallengeMethod for
